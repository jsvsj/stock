...
class MABacktestGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("均线策略回测系统（20均线向上过滤）")
    ...
    def _create_widgets(self):
        ...
        # 新增：20均线向上判断周期（默认3天，即ma20今日>3日前）
        ttk.Label(input_frame, text="20均线向上周期：").grid(row=2, column=6, sticky=tk.W, padx=5, pady=5)
        self.ma20_up_period = tk.StringVar(value="3")
        ttk.Entry(input_frame, textvariable=self.ma20_up_period, width=10).grid(row=2, column=7, padx=5, pady=5)
        ...
        # 1.4 执行按钮
        ttk.Button(input_frame, text="开始回测", command=self.run_backtest).grid(row=4, column=0, columnspan=8, pady=10)
        ...
        # 2.2 可视化结果
        plot_frame = ttk.LabelFrame(result_frame, text="收益曲线（标注最大回撤）", padding=10)
    ...
    def _validate_params(self):
        ...
        try:
            ...
            ma20_up_period = int(self.ma20_up_period.get())  # 新增周期验证
            if ma5 <= 0 or ma7 <= 0 or ma20 <= 0 or ma20_up_period <= 0:
                raise ValueError("均线参数和向上周期必须为正整数")
            ...
            return {
                ...
                "ma7": ma7,
                "ma20": ma20,
                "ma20_up_period": ma20_up_period,  # 新增：20均线向上周期
    ...
    def _get_stock_data(self, params):
        ...
        try:
            ...
            # 新增：计算20日均线向上信号
            # 逻辑：今日ma20 > N日前ma20（N=ma20_up_period），代表20均线向上
    ...
    def _calculate_signals(self, df, params):
        """计算交易信号（新增20均线向上过滤）"""
        # 1. 初始入场：5/20金叉 + 空仓 + 20均线向上
        df["golden_cross_520"] = (df["ma5"].shift(1) < df["ma20"].shift(1)) & \
                                 (df["ma5"] > df["ma20"]) & \
                                 df["ma20_up"]  # 新增：20均线向上

        # 2. 离场：5/7死叉 + 持仓（逻辑不变）
        df["death_cross_57"] = (df["ma5"].shift(1) > df["ma7"].shift(1)) & (df["ma5"] < df["ma7"])

        # 3. 再入场：20日均线之上 + 5日均线上穿7日均线 + 20均线向上
        df["golden_cross_57"] = (df["close"] > df["ma20"]) & \
                                (df["ma5"].shift(1) < df["ma7"].shift(1)) & \
                                (df["ma5"] > df["ma7"]) & \
                                df["ma20_up"]  # 新增：20均线向上
        return df
    ...
    def _calculate_max_drawdown(self, equity_curve):
        ...
        peak = equity_series.expanding().max()  # 累计峰值
        drawdown = (equity_series - peak) / peak * 100  # 回撤百分比
        max_drawdown = drawdown.min()  # 最大回撤（负数）
        # 找到最大回撤对应的日期索引
        max_dd_idx = drawdown.idxmin()
        return max_drawdown, max_dd_idx
    ...
    def _backtest_strategy(self, df, params):
        ...
        for idx, row in df.iterrows():
            ...
            # 再入场：5/7金叉 + 空仓 + 20日均线之上（已过滤20均线向上）
        ...
        # 计算最大回撤
        max_drawdown, max_dd_idx = self._calculate_max_drawdown(equity_curve)

        return {
            "total_return": total_return,
            "max_drawdown": max_drawdown,
            "max_dd_idx": max_dd_idx,  # 最大回撤点索引
    ...
    def _calculate_hold_return(self, df, params):
        ...
        for idx, row in df.iterrows():
            ...
            hold_equity.append(current_equity)

        # 计算持有不动的最大回撤
        hold_max_dd, hold_max_dd_idx = self._calculate_max_drawdown(hold_equity)
        # 归一化净值曲线（用于绘图）
        hold_equity_norm = [e / initial_hold_cash for e in hold_equity]

        return {
            "total_return": hold_total_return,
            "max_drawdown": hold_max_dd,  # 新增：持有不动最大回撤
            "max_dd_idx": hold_max_dd_idx,  # 新增：最大回撤点索引
            "final_equity": hold_final_equity,
            "equity_curve": hold_equity_norm,
            "raw_equity_curve": hold_equity  # 原始净值曲线（用于计算）
        }
    ...
    def _plot_result(self, df, strategy_equity, hold_equity, params):
        ...
        # 绘制基础曲线
        strategy_line = self.ax.plot(df["trade_date"], strategy_equity_norm,
                                     label=f"波段策略（收益率：{self.backtest_result['total_return']:.2f}% | 最大回撤：{self.backtest_result['max_drawdown']:.2f}%）",
                                     color="red", linewidth=1.5)
        hold_line = self.ax.plot(df["trade_date"], hold_equity,
                                 label=f"持有不动（收益率：{self.hold_result['total_return']:.2f}% | 最大回撤：{self.hold_result['max_drawdown']:.2f}%）",
                                 color="blue", linewidth=1.5)

        # 标注波段策略最大回撤点
        strat_dd_idx = self.backtest_result["max_dd_idx"]
        self.ax.scatter(df.iloc[strat_dd_idx]["trade_date"], strategy_equity_norm[strat_dd_idx],
                        color="red", s=50, marker="*", label=f"波段策略最大回撤点")

        # 标注持有不动最大回撤点
        hold_dd_idx = self.hold_result["max_dd_idx"]
        self.ax.scatter(df.iloc[hold_dd_idx]["trade_date"], hold_equity[hold_dd_idx],
                        color="blue", s=50, marker="*", label=f"持有不动最大回撤点")

        # 美化图表
        self.ax.set_title(f"{params['stock_code']} 均线策略回测收益曲线（20均线向上过滤）", fontsize=12)
...
20均线向上周期：{params['ma20_up_period']}天（今日MA20 > {params['ma20_up_period']}日前MA20）
...
【对比结论】
收益对比：{('波段策略收益更高' if self.backtest_result['total_return'] > self.hold_result['total_return'] else '持有不动收益更高')}
收益差值：{abs(self.backtest_result['total_return'] - self.hold_result['total_return']):.2f}个百分点
风险对比：{('波段策略回撤更小' if self.backtest_result['max_drawdown'] > self.hold_result['max_drawdown'] else '持有不动回撤更小')}
回撤差值：{abs(self.backtest_result['max_drawdown'] - self.hold_result['max_drawdown']):.2f}个百分点
"""
    ...
    def run_backtest(self):
        ...
        messagebox.showinfo("回测完成", "策略回测已执行完毕（含20均线向上过滤），结果已展示！")
